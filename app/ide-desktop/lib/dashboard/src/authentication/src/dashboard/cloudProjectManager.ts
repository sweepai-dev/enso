...
import * as projectManager from '../../../../../content/src/project_manager'
...
export class ProjectManager implements Omit<projectManager.ProjectManager, 'connectionUrl'> {
    // FIXME[sb]: Backend MUST change when e.g. a different user logs in.
    constructor(public element: JSX.Element, public backend: backend.Backend) {}

    async openProject(
        params: projectManager.OpenProjectParams
    ): Promise<projectManager.Result<projectManager.OpenProject>> {
        const project = await this.backend.getProjectDetails(params.projectId)
        const ideVersion = (
