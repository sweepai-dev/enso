...
import org.enso.compiler.core.IR.Application.{ForceDefaults, ForceThunk}
  ...
  sealed private class ExpressionProcessor(
    ...
    def processApplication(application: IR.Application): RuntimeExpression =
      application match {
        ...
        case IR.Application.Force(expr, ForceThunk, location, _, _) =>
          setLocation(ForceNode.build(this.run(expr)), location)
        case IR.Application.Force(expr, ForceDefaults, location, _, _) =>
          setLocation(ForceDefaultsNode.build(this.run(expr)), location)
